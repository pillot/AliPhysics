		    ...
		    Bool_t applyAccCut, Double_t minMomentum, Bool_t correctForSystematics, Int_t extrapMode, Int_t nevents, TString extraLibs)
{
  ...
  for (Int_t iStep = 0; iStep < nSteps; iStep++) {
    ...
    // clean memory
    mgr->SetAnalysisType(AliAnalysisManager::kLocalAnalysis); // to make sure the all output containers are deleted
    delete mgr;
...
void LoadAlirootOnProof(TString& aaf, TString alirootVersion, TString& extraLibs, Int_t iStep)
{
  ...
  TString nWorkers = (aaf == "caf") ? "workers=20" : "";
						   ...
						   Double_t clusterResNB[10], Double_t clusterResB[10])
{
  ...
  if (selectPhysics) {
    ...
    AliPhysicsSelectionTask* physicsSelection = reinterpret_cast<AliPhysicsSelectionTask*>(gROOT->ProcessLine("AddTaskPhysicsSelection()"));
    if (!physicsSelection) {
  ...
  AliCentralitySelectionTask* centralityTask = reinterpret_cast<AliCentralitySelectionTask*>(gROOT->ProcessLine("AddTaskCentrality()"));
  if (!centralityTask) {
  ...
  else */muonResolution->SetDefaultStorage("alien://folder=/alice/data/2010/OCDB");
